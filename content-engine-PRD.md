...
> **Revision note, 2026-08-23. Retirement becomes per-profile suppression, and the build stops waiting on one person.** Third pass from `content-engine-CORRECTION.md`, whose decision 4 is overturned the same day. One defect in two places, §9's retirement rule and that decision, and both said the same thing: a gate rule that over-fires gets struck out of `reference/ai-tells.md`, globally and permanently, on evidence from Joseph's corpus, and nothing may be struck until the negative control has run on his samples. `reference/ai-tells.md` ships to everyone who clones this repo, per §1.2, so striking a rule there because it over-fires on one person's punctuation writes that person's habits into every future install. It is a portability defect in the costume of a calibration step, and it is §1.0's G5 failing — the commitment the review graded D. Rules are no longer retired for over-firing. They are **suppressed per profile**, in `profiles/<handle>/gate-calibration.md`, with the evidence that did it, and the shipped file stays identical for everyone with every rule on. Changed: §1.0 (the precedence rule's second application to §9), §5 (`gate-calibration.md` in the profile layout and in the gate-time load line), §6.2 (setup runs the negative control on the pasted samples and writes the calibration), §7a (the new file's write rules, and the one place its schema departs from the `source:` rule), §9 (the retirement rule becomes the suppression rule, the maintenance path keeps strikethrough only for a rule that is wrong everywhere, punctuation density is unblocked, the prediction is reframed), §13 (phases 1.3 and 2a leave the build order and become features of setup; third cut of that table, with the history of all three), §13.2 step 4 (the pass bar is the invariant every setup establishes, not a build acceptance test), §15 (decision 4). **Section numbers are unchanged, and so are §13.2's step numbers.** The phase ids are the schedule; §13.2's step numbers remain the stable index of what done means.
>
> **Also folded into the same pass, 2026-08-23, from the build of `workflows/review.md` and `workflows/connect.md`.** Seven mechanical defects, each one a spec sentence the workflow could not implement as written. §3 (the staleness row claimed every profile file carries `last_reviewed` while three of the five with declared intervals carried no such field, and the quarterly check was the only stamper, which left `inventory.md` and its 30-day interval permanently stale; the stampers are now allocated per file and `workflows/review.md` owns them and defines the three questions §3 has always named and never enumerated), §5 and §1.2 (`connect` was missing from both `workflows/` lists), §7c (the connector scope was "declared once," which left no change path; `workflows/connect.md` specifies re-run, print, line diff, accept), §11 and §13.2 step 7 (both asked for five values at review while the score divides by follower count and the frozen header carries `followers_at_post`, so the divisor had no collection path; both ask for six now), §16 (§11's exact Mann-Whitney U has no implementation in `engine.js`, recorded as a deferral with `workflows/review.md`'s hand-checkable mitigation and a step-15 trigger, and the follower-normalization deferral is halved because the capture gap is closed). One reported claim did not survive checking and no edit was made for it: `profiles/_template/inspiration.md` does not carry `last_reviewed`, it carries the sentence saying it has none, which is what §3 already said.
...
That precedence is written down rather than assumed because the failure it names has already happened once, and it happened by the book. §13's build order was ordered on "prove the cheap format before building the expensive one," which was the right rule when all four formats were unbuilt. By 2026-08-19 three of the four ran, and the rule was holding all three behind a count of ten shipped short posts. Every step in that order conformed to this document. G1 graded D anyway. A specification that can be followed exactly into a miss needs a clause saying which one wins when they part, and this is that clause: the commitment wins, and the section is the thing that gets rewritten. §13 is the first section that rule was applied to; §4 and §9 are the second and third. **Applied to §9 a second time on 2026-08-23**, and on G5 rather than G1: §9's retirement rule conformed to this document exactly and would have shipped one person's punctuation habits to every install that ever gets cloned. §9's suppression rule is what replaced it.
...
| Stale files | **Five files carry `last_reviewed`, and every one of them has a stamper. Corrected 2026-08-23.** This row read *"Every profile file carries `last_reviewed`"* and it was wrong in the direction that costs something: three of the five files with a declared interval carried no such field, `thesis.md`, `voice.md` and `inventory.md`, so each could be flagged stale and never cleared. Per-file intervals rather than one global number: identity 180 days, audience 120, voice 90, thesis 90, inventory 30. `inspiration.md` and `theme.json` carry no `last_reviewed` at all, because a creator you liked in February is still a creator you like and a hex code does not go stale, and neither do `learnings.md`, `shipped-history.md`, `inventory-archive.md`, `connectors.md` or the run ledger. Only files the current run actually loaded are flagged. Staleness gets at most one line of run-start output, most urgent wins. **The stampers, allocated rather than assumed:** the quarterly three questions stamp identity, audience and thesis; `voice.md` is stamped by accepting a re-derivation of its four measurements, shown as an explicit diff; `inventory.md` is stamped by a completed weekly top-up. That last one is the reason this row changed: it carries the shortest interval in the system at 30 days and the old rule gave the quarterly check as the only stamper, so the file with the tightest clock had no way to clear it a month after setup. `workflows/review.md` owns all five and is also where the three questions are defined, since this section names them and has never enumerated them; `SKILL.md` step 1 carries the same allocation as a table. Nothing stamps a file because a command merely read it. |
...
**In:** the router and profile structure, the setup interview, `reference/engine.js`, the language gate with its fixtures, short posts end to end, brief front matter and the ship keystroke, the repetition guard, a minimal `/content-engine review`, both vendored renderers (the infographic's thumbnail, tint and contrast checks land 2026-08-24; the carousel has copy budgets only, and neither renderer has the line-overflow check, refused with its reason in `VISUALS.md` §6.3), carousels and the design gate, infographics, articles as newsletter editions, and the repurpose refusal.
...
content-engine/
  ...
  workflows/                     # setup, short-post, article, carousel, infographic, review, connect
  reference/
  ...
  profiles/
    ...
    <handle>/                    # e.g. joseph/, or jane-ibm/
      ...
      gate-calibration.md        # which §9 rules are suppressed for this person, and the
                                 #   evidence that suppressed them. written by setup,
                                 #   re-measured at review. added 2026-08-23.
...
> identity.md, voice.md, thesis.md, audience.md, learnings.md (entries tagged `confirmed` only), the compact inventory index with only shortlisted items hydrated, and inspiration.md only when voice.md declares that no samples exist. `ai-tells.md` loads at gate time, not at draft time, and `gate-calibration.md` loads with it, because the gate cannot read the tell list correctly for this person without knowing which rules that person's own writing already disproved. It does not read the render templates, the design gate, or the inventory archive.
...
**Then calibrate the gate against those samples, here, before any draft is generated. Added 2026-08-23.** Run `engine.js gate --negative <profile>` over what was just pasted and write `profiles/<handle>/gate-calibration.md`: every §9 rule that rewrote a word in the person's own unedited writing is suppressed for this profile, with its fire count and the sample that fired it. Nothing in `reference/ai-tells.md` is edited, ever, per §9's suppression rule; that file is identical in every install. This runs before the calibration post at the end of this session rather than after it, because the gate that post runs through has to already be this person's gate. Otherwise the first thing they watch the engine do is rewrite the way they write, which is the one impression the whole section exists to avoid. An absent calibration means nothing is suppressed and every rule fires, which is the correct default and is never a clean pass.
...
**What "profile file" means.** The curated files listed in §5: identity.md, inventory.md, inventory-archive.md, voice.md, inspiration.md, audience.md, thesis.md, theme.json, learnings.md, shipped-history.md, gate-calibration.md, connectors.md. `runs/` and `analytics/` are the run ledger, and every run appends to them. That is the whole of §1.3's write scope: profile files are governed here, the ledger is written by every run.
...
**`gate-calibration.md`, added 2026-08-23.** Setup writes it, from the negative control over the samples pasted in §6.2, and `/content-engine review` rewrites it on a re-measure. It is the one profile file whose entries carry evidence in place of `source:`: a suppressed rule records the `fired:` count and the `sample:` that fired it, which is a stronger provenance claim than any of the four values above, and it is written down here so the departure is deliberate rather than a rule violation nobody noticed. No line in it is ever hand-edited. A rule is suppressed by measurement or it is not suppressed, per §9. A re-measure follows the re-derivation rule below, engine shows the diff and does not write until you accept, because a re-measure over a wider corpus can put a suppressed rule back, and a rule coming back on has to be a decision somebody made rather than a side effect of running a weekly command.
...
**Allowlist only. "All of Slack" is not a scope and is never offered.** Named channels plus a named Gmail label or query, **declared and revisable** by `/content-engine connect` and written to `profiles/<handle>/connectors.md`. **Reworded 2026-08-23:** this read "declared once," which left a scope with no change path, and `workflows/connect.md` specifies one: re-run the command, print the current scope, show a line diff, write only on accept. That file exists so the scope can be audited by opening it, in five seconds, without reading any code. It is not loaded at draft time and it is not in §5's load list.
...
**Precedence: voice.md wins.** A tell that contradicts a documented habit in voice.md does not fire for that profile. The gate records the override in gate-report.md instead of rewriting. A shared, person-independent blocklist does not get to silently overwrite the one file that knows what this person sounds like. **The suppression rule below is this paragraph made mechanical, 2026-08-23.** voice.md documents a habit in prose and a model has to notice it; gate-calibration.md records the measurement and the engine reads it. Same precedence, two instruments, and the second one cannot forget.
...
**Three checks join the deterministic half at §13's Phase 2**, each a count rather than a judgment, each floored against the person's setup baseline with an absolute fallback where no baseline exists. **Burstiness**, the coefficient of variation of sentence length; `engine.js stats` already returns both stdev and mean, so it is one division. **Punctuation density**, commas, semicolons, parentheses and dashes per 100 words. **Nominalisation rate**, `-tion`, `-ment`, `-ness`, `-ity`, `-ance` and `-ence` per 100 words. Punctuation density is the awkward one: it is the strongest 2026 tell, and this gate as written actively enforces it by banning two of the four marks. All three are redraft-class, per the list above. **The three no longer arrive together. Revised 2026-08-20.** This passage read that punctuation density "is the reason the three arrive together," and it is instead the reason one of them arrives later. Burstiness and nominalisation depend on nothing the retirement rule below can strike, so they are specified and built on this section as written, at §13's Phase 2c. Punctuation density is specified after the retirement pass, at §13's Phase 2d, because a density floor and a zero-tolerance ban on two of its own four inputs cannot both bind, and which of those two bans survives is the negative control's verdict to give. **Punctuation density is unblocked, 2026-08-23.** It was held behind a global retirement and there is no longer one to wait for. Under the suppression rule the floor is per profile like every other distribution check in this section: it counts against the person's own baseline, and where a ban is suppressed for that person the mark is theirs to use, so it counts toward their density rather than against a rule that no longer fires on them. Where a ban is live for someone, the check floors on the marks they actually use, which is the same absolute-fallback logic burstiness and nominalisation already carry. Nothing about it is downstream of a corpus that does not exist yet, so all three are specified and built together after all, at §13's Phase 2c. **Register inflation goes to the model, not the engine**, because it is a judgment, and this section's honesty rule is that zero tolerance is a promise only the deterministic half can keep.

**Maintenance. Revised 2026-08-23.** When Joseph spots a new tell in the wild, he says so and it gets appended with the date. The rest of this paragraph read *"Entries can also be retired: a rule that fires on the person's own writing, or on an inspiration.md post, gets struck through with the date and the reason, so it stops firing and stays visible."* A rule firing on one person's writing is no longer an edit to this file at all. It is a suppression in that person's `gate-calibration.md`, per the suppression rule below. **Strikethrough survives for the one case it was always right about: a rule that is wrong about writing rather than wrong about a person.** A pattern that matches nothing across `gate-fixtures/`, or that matches something every profile which measured it produced unedited, is a bad rule everywhere, and it gets struck with the date and the reason so it stops firing and stays visible. The test to apply is whether striking it would be correct for a stranger who has never used this engine. A rule that turns out to be wrong costs a rewrite of good writing on every future draft forever, and precision can only decay, which now cuts both ways: a rule struck for one person's habit decays precision for every other install at the same time.

**The suppression rule. Replaced 2026-08-23.** From 2026-08-19 this was the retirement rule and it read: *"Nothing in this section is retired until `gate --negative profiles/joseph` has run on real samples. The maintenance path above is the mechanism, and this rule is the precondition on using it."* The precondition was right and the mechanism was wrong. `reference/ai-tells.md` ships with the engine, per §1.2, so striking a rule there because it over-fires on one person's punctuation writes that person's punctuation habits into every future install, and whoever clones the repo in November inherits a gate with a hole in it, no evidence in front of them, and no way to know the hole is about somebody else. That is G5 failing in the place the plan of record was most confident, and per §1.0 the commitment outranks the section.

**Rules are never retired for over-firing. They are suppressed per profile.** `reference/ai-tells.md` ships identical to every install with every rule on. Each person's setup runs the negative control over their own pasted samples, per §6.2, and writes `profiles/<handle>/gate-calibration.md`, which records every rule that rewrote a word they wrote, its fire count, and the sample that fired it. The gate reads that file on every run. Five conditions, and together they are the contract:

1. **Evidence or nothing.** A rule is suppressed only by a `fired:` count above zero on that person's own corpus, with the sample named. There is no hand-edited suppression, because a suppression somebody typed is a rule switched off by preference and it is indistinguishable in the file from one switched off by measurement.
2. **Absence means everything fires.** No calibration file, or no entry for a rule, means that rule is live for that profile. A missing calibration is never read as a clean pass, for the same reason §13.2 step 4's empty corpus fails loudly: a rule looks perfectly innocent when nothing was measured against it.
3. **Reported, never silent.** `gate --report` names what this profile suppresses, and `gate --fixtures` states any suppression in force when it reports recall, because recall measured over a suppressed rule is a number the engine did not earn. That is §13.1's self-witnessing failure arriving by a new route.
4. **Only the repair class is suppressible.** A `reject`, meaning clearance and private terms, is never suppressible by anyone: what it guards is not a matter of taste, and no volume of evidence about a person's punctuation is evidence about their NDA. A `redraft` or a `flag` edits no word the person wrote, so it has nothing to suppress; it is reported and the human decides.
5. **The shipped file and the tell table stay in lockstep.** `engine.js gate --tells` drift is unaffected, because neither `ai-tells.md` nor the table changes per person. Suppression is a third file the gate consults, not an edit to either of the two it already reconciles.

**What survives from the retirement rule is the reasoning that produced the negative control**, and it is why the control exists at all: a prediction made from someone else's corpus is persuasive enough to act on and is not evidence about this person, so only the negative control can settle whether a rule over-fires. What failed was the inference from that to a global edit. The verdict is about one writer, so it belongs in that writer's profile, and the file everybody downloads is the last place to write it down.

**The prediction, recorded as a prediction.** Run over 12 human-published articles in a separate repo, this gate demands 64 rewrites: em dash 36, rhetorical fragment 11, semicolon 9, Title Case header 4, and banned-lexicon, en dash and antithesis 4 between them. That is a proxy corpus and a different writer, so it is the strong prediction and it is not the authority. Joseph's own samples are. **Reframed 2026-08-23:** this read as the prediction "about which rules will die," and no rule dies. It predicts which lines Joseph's `gate-calibration.md` will carry and nothing beyond that. The same list run against a different person predicts a different calibration, which is the entire reason the calibration is per profile rather than per repo. If his corpus disagrees with this list, his corpus is right about him and the list is a coincidence about another person's punctuation.
...
**Superseded 2026-09-06.** Both renderers were retired to `archive/2026-09-06-html-renderers/` and replaced by `render/imagegen/render.js`, which calls an image-generation model through OpenRouter instead of drawing the pixels. What still holds from the paragraph above: **Claude writes a Markdown spec, never HTML**, the grammar of that spec is unchanged, and the carousel still ships one PDF uploaded as a document post. What changed: there is no HTML artifact at all, so nothing sits beside the output for the gate to read and the gate reads the image; the infographic is one PNG at **1152x1536, 3:4** rather than 2160x2700 at 4:5, because the model's route does not offer 4:5; the carousel PDF is a raster page per slide at the same 3:4 geometry rather than a vector print at 810x1013pt. The no-hand-editing rule survives intact and now reads: never patch a generated image, regenerate it from an edited prompt. Full reasoning in `docs/superpowers/specs/2026-09-06-imagegen-infographics-design.md`.

**theme.json's keys**, which are the documented input contract §1.1 refers to: `bg`, `fg`, `accent`, `muted`, `font_head`, `font_body`, `scale` (dense | airy), `radius`, `rule_weight`. Nothing else. `_template/theme.json` ships with defaults that clear the design gate unedited, because the first visual run writes the real values. One addition is proposed and not settled, and it is not made in passing elsewhere: a second semantic accent, `accent_2`. It is §15 decision 9. An attribution-posture flag was proposed as a tenth key in the same pass and withdrawn, because two sections were independently adding a different tenth key to a nine-key contract and only one of them earns it; the signature in §10.3 is hardcoded, and anyone who wants a CTA bar can ask and get one edit. **A tenth key is admitted 2026-08-20, and it is `logo`. Optional, empty by default, a path resolved inside `profiles/<handle>/`.** The withdrawal above was right about a proposal and this is not one: Phase 5 found `render/infographic/render.py` hard-requiring a brand logo file to render anything at all, so without this key the portable engine cannot draw a frame for anybody who is not the company whose logo shipped in it. That is the necessity test the withdrawn flag failed and this passes. **A second candidate, `wordmark`, is declined on the same test**, because the carousel renders correctly without it, showing the accent disc and no name, and a deck that wants a signature sets the footer in its own spec front matter. So the contract is nine keys plus one optional path, and `accent_2` is still §15 decision 9. **An eleventh key is admitted 2026-09-06, and it is `style_reference`. Optional, empty by default, a path resolved inside `profiles/<handle>/`, exactly `logo`'s shape.** It passes the same necessity test both earlier candidates were held to: the image-gen renderer's only channel for "make it look like this" is a reference image, and without a key naming one, every profile's visual identity is whatever a paragraph of hex codes produces. Absent, the model gets the text description alone, which is correct and not a degraded state, the same as an absent `logo`. `render/imagegen/reference/themes.md` holds the contract. **Three of the nine are now read by nothing**: `scale`, `radius` and `rule_weight` are CSS layout tokens with no image-gen equivalent, and they are left in the schema rather than removed, so no profile needs migrating and `accent_2` inherits an unchanged nine-key core if it ever ships.
...
`/content-engine review` rolls this up weekly. It also: lists run slugs with no posts.csv row, oldest first, and asks for URL plus `followers_at_post`, reactions, comments, reposts, and `run_again` (**six values, corrected 2026-08-23:** this list and §13.2 step 7 both asked for five, and the score below divides by follower count, so the frozen header's `followers_at_post` column had no collection path and the divisor was never captured); prompts on any brief still `drafted` after 7 days ("Did you ship 2026-10-23-forecast-carousel?"); runs the primed top-up once; asks "anything ship outside the engine? paste it," which back-fills the brief and the diff corpus for anything written without the router; and once a quarter asks the three staleness questions, which §3 allocates and `workflows/review.md` defines.
...
| Phase | What | Owner | Est | Blocks |
| :- | :- | :- | :- | :- |
| 0 | Instruments: the `stripFences` comment, `format` de-hardcoded, `overlap` given an invocation site | subagent | done 2026-08-19 | nothing |
| 0.5 | This document's structural rewrite, so the spec leads the work rather than trailing it | subagent | done 2026-08-19 | nothing |
| 2a | The suppression layer: setup runs the negative control and writes `profiles/<handle>/gate-calibration.md`, the gate reads it on every run, `--report` and `--fixtures` name what it suppressed | subagent | ~1h | nothing |
| 2b | Invert the mechanism: the `action:` column, bound 3's new job, the single redraft, the better-of-two comparator | subagent | ~1h | nothing |
| 2c | §9's three new checks: burstiness, nominalisation, punctuation density | subagent | ~45m | nothing |
| 3 | `reference/reader.md`, the grader | subagent | ~1h | nothing |
| (pause) | **Checkpoint: Joseph reviews** | Joseph | | |
| 5 | Vendor both renderers; write `workflows/{infographic,carousel,article}.md` | subagent | ~1d | nothing |
| 1a | Paste front door: three to five unedited samples into `voice.md` and `shipped-history.md`, §6's four measurements frozen off them, `gate-calibration.md` written from them | **Joseph** | ~10m | nothing |
| 1b | The real setup interview, session one | **Joseph** | 30-45m | nothing |
| 4 | Deep interview, chunked into 30-45m sessions each ending in a visible improvement, continuing 1b | **Joseph** | async | nothing |
| 6 | Evidence pass on this document: the measured thresholds, and Joseph's calibration recorded as the first one | subagent | ~1h | nothing |

**Nothing in this table blocks anything else in it.** That column is one word repeated on purpose, because it is the whole result of the 2026-08-23 cut and the previous two cuts were both arguments about how wide one dependency reached.

**The engine is built and packaged first, and the interview is the last step. Re-cut 2026-08-23.** Anything personal, meaning Joseph's corpus, his posts and his profile, comes after the engine ships rather than as a dependency of building it, so every Joseph row moved below Phase 5 and the two rows that existed only to consume his corpus are gone from the build. **Phases 1.3 and 2a stop being build phases and become features of setup.** 1.3 was one run of `gate --negative profiles/joseph`, and 2a was retiring what over-fired on him; both are now one shipped capability that runs for every person who installs the engine, built once at the new 2a and run at 1a, at every other install's 1a, and again at every re-measure. The serial chain the whole table was organised around, 1a to 1.3 to 2a to 2d, does not exist: there is no 1.3, no retirement, and 2d merges into 2c because punctuation density was held behind the retirement and has nothing left to wait for. See §9's suppression rule.

**2a keeps its id and changes its content, deliberately.** It was the retirement pass and it is now the suppression layer. `reference/reader.md` already cites *"correction Phase 2a"* as the phase that makes the em-dash ban per-profile suppressible, so that citation is correct against this table and would have been broken by a fresh id, and there is no surviving retirement work left for a later reader to confuse it with. Nothing else in this document renumbers: §13.2's step numbers are untouched, as always.

**Phase 4 stops running in parallel with Phase 5 and runs after it**, which is the same principle stated the other way round: **inventory depth gates draft quality, not format support**, so draft quality is the thing you improve on a finished engine rather than a thing you build one around.

**This is the third cut of this table, and the history is the point.** A table re-cut three times with no visible history is how a plan quietly loses its authority, so all three reasons stay readable here. **The first cut, 2026-08-19**, replaced sixteen steps with phases, because the ordering principle was "prove the cheap format before building the expensive one" and it was holding three working formats behind a count of ten shipped posts while G1 graded D; the note above the table holds it. **The second cut, 2026-08-20**, split Phase 1 into 1a and 1b and Phase 2 into 2a through 2d, because one dependency had been drawn around both whole phases, and the code said otherwise: `gateNegative` opens `voice.md` and `shipped-history.md` and nothing else (`reference/engine.js:1067-1071`), never `identity.md`, `thesis.md`, `audience.md`, `inventory.md` or `learnings.md`, every one of which is an interview output. So the paste gated the negative control and the interview did not, which narrowed a 45-minute conversation to a ten-minute paste. That reading is still correct and it is now moot. **The third cut, today**, is the one the first two kept walking past: the dependency should not have existed. Both earlier cuts argued about how wide Joseph's corpus reached, and neither asked what it was feeding, which was a strikethrough in a file that ships to every install. The artifact was wrong, so the dependency was never a scheduling problem. What survives all three cuts unchanged: neither 1a nor 1b is delegable, because both are Joseph talking.
...
- **Recall is counted over engine-owned tells only.** `engine.js gate --tells` is the authority on which half owns each tell, and it fails when `ai-tells.md` and the table disagree. Scoring a model-judged tell as caught because a fixture lists it reports a number `engine.js` did not earn, which is the self-witnessing problem §13.1 exists to stop. **And recall states what the profile suppressed, added 2026-08-23.** When `--fixtures` runs with a profile in scope, it names every rule that profile suppresses, because recall measured over a suppressed rule is the same unearned number arriving by a different door. Suppression is per profile, per §9, so the shipped rule set the fixtures measure is the full one and the two numbers can never be quietly mixed.
- The negative control runs in the same pass: the gate over the person's own pasted samples and the inspiration posts. **Zero rewrites on the person's own samples is a per-profile invariant, not a build acceptance test. Reworded 2026-08-23.** This condition read *"Zero rewrites on the person's own samples is the pass bar"* and it sat here as a thing step 4 had to demonstrate once, on one person, which is what made one man's corpus a dependency of shipping the gate. It is the bar every setup has to reach for the person it just interviewed, at every install, forever, and it is re-established on every re-measure. What step 4 owes is the mechanism that reaches it, and that mechanism is testable today with no corpus: a `gate --negative` over `_template`'s empty Samples section fails loudly, a run over a fixture profile carrying a planted em dash writes a calibration suppressing `em-dash` with its count and its sample, and a gate run reading that calibration stops firing that rule for that profile and says in `--report` that it did.
- **What "rewrites" buys is unchanged and still the operative word rather than "fires".** A rewrite edits a word they wrote, so each one is either a rule that is wrong about this person, which is a suppression in their `gate-calibration.md`, or a rule that is wrong about everyone, which is §9's maintenance path and a strikethrough. A specifics-floor redraft and a voice floor edit nothing, so they are reported and suppress nothing. The absence checks run only on samples whose `kind:` is a post, because they measure a whole post and running them on three lines of Slack suppresses a real rule over a sample it never claimed. On the inspiration corpus, count how many rewrites the person judges worse; more than 2 of 20 means the rule that fired is over-firing. **That half lands on the maintenance path rather than in the calibration, 2026-08-23:** the inspiration posts are somebody else's writing, so what they show is the rule's precision rather than a fact about this person, and a rule that mangles a stranger's good prose is a candidate for strikethrough rather than for suppression in this one profile. Two hours, no distributions. **A negative control that reads an empty corpus fails loudly rather than reporting a pass**, added 2026-08-19 and load-bearing under the new model rather than merely retained: §9's suppression rule reads an absent calibration as nothing suppressed, so the one thing that must never happen is a control that reports zero fires over nothing read, which is the same false statement as a clean gate report on a draft nobody scanned.
...
- No report-only mode for the first ten posts: it teaches the user on day one that the gate is something you supervise, and the negative control suppresses the rules that are wrong for this person before they touch a real draft.
...
- Lists run slugs with no posts.csv row, oldest first. Asks for URL plus `followers_at_post`, reactions, comments, reposts, and `run_again`. Six values, not five, added 2026-08-23: `followers_at_post` is in the frozen header and §11's score divides by it, so a five-value ask makes the score uncomputable on every row it writes.
...
4. **Profile.** Joseph's profile comes from the real setup interview, not from a bootstrap off anyone else's writing OS. This is what makes his corpus the instrument that retires gate rules, per §9's retirement rule, and it is why §13's Phase 1 cannot be delegated. **Revised 2026-08-20:** Phase 1 is now two rows, 1a the paste and 1b the interview. Neither is delegable and only 1a is a gate. **Revised 2026-08-23:** the first sentence stands and everything after it is overturned, with the correction's decision 4. His corpus is the instrument that calibrates *his* gate and it retires nothing, because nothing is retired: §9's suppression rule keeps `reference/ai-tells.md` identical in every install and records over-firing in `profiles/<handle>/gate-calibration.md`. Neither 1a nor 1b is a gate on anything in §13 any more, and both moved below the Phase 3 checkpoint and below Phase 5, because the engine is packaged before it is pointed at a person.
...
**Follower normalization in the score.** §11 divides by follower count and never captures it, and the divisor drifts against a rolling window. Deferred because §11 is post-MVP and the fix is a deletion: `rel_score = raw_score / median(raw_score, previous 20 posts)`, which is what §11's own threshold sentence already speaks in. **Trigger:** step 15. Apply it in the same pass and make §12's engagement bullet read `rel_score` verbatim, so §11 and §12 stop using two different denominators. **Half of this is closed as of 2026-08-23:** "never captures it" was true of the ask lists rather than of the header, and §11 and §13.2 step 7 now both ask for `followers_at_post`, so the divisor exists. What is still deferred is the drift, which is the half the deletion fixes.

**The exact Mann-Whitney U has no implementation.** §11's learning threshold is an exact U on ten scores qualifying at U <= 2, and `engine.js` has five subcommands, none of which computes it. That is the failure §13.1 was written about: an exact statistic handed to attention fails silently and reports clean, and here it would write a `hypothesis` line into learnings.md that nothing later distinguishes from an earned one. Deferred rather than built now because §11 is post-MVP and nothing loads a `hypothesis` entry at draft time, so an unearned one cannot reach a post. **The standing mitigation is `workflows/review.md`'s**, and it is why this is a deferral rather than a defect: review quotes the ten scores and the pairwise count, so the U is checkable by hand on the numbers the run printed. **Trigger:** step 15, in the same pass as the two entries above. Then `engine.js` owes a sixth subcommand rather than a prose instruction, on the same argument that put the other six mechanisms in that file.
